...
class CMUser extends CObject implements IHasSQL, ArrayAccess {
   ...
   * Constructor
   */
  public function __construct($ra=null) {
    parent::__construct($ra);
  ...
  public function Init() {
    try {
      ...
      $password = $this->CreatePassword('mazzan');
      $this->db->ExecuteQuery(self::SQL('insert into user'), array('mazzan', 'Mazzan', 'mazzan@masoft.se', $password['algorithm'], $password['salt'], $password['password']));
      $idMazzanUser = $this->db->LastInsertId();
      ...
      $this->db->ExecuteQuery(self::SQL('insert into user2group'), array($idMazzanUser, $idAdminGroup));
      $this->db->ExecuteQuery(self::SQL('insert into user2group'), array($idMazzanUser, $idUserGroup));
  ...
  public function CreatePassword($plain, $algorithm=null) {
    $password = array(
      'algorithm'=>($algorithm ? $algoritm : CRama::Instance()->config['hashing_algorithm']),
      'salt'=>null
    );

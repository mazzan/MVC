<?php
/**
 * A model for managing Rama modules.
 * 
 * @package RamaCore
 */
class CMModules extends CObject {
   ...
   * Properties
   */
  private $ramaCoreModules = array('CRama', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $ramaCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isRamaCore']   = in_array($rc->name, $this->ramaCoreModules);
      $details['isRamaCMF']    = in_array($rc->name, $this->ramaCMFModules);

<?php
/**
 * Main class for Rama, holds everything.
 *
 * @package RamaCore
 */
class CRama implements ISingleton {
	...
	protected function __construct() {
		...
		$ra = &$this;
    require(RAMA_SITE_PATH.'/config.php');
  ...
  /**
	 ...
	 * @return CRama The instance of this class.
	 */
	public static function Instance() {
		if(self::$instance == null) {
			self::$instance = new CRama();
		}
  ...
  public function ThemeEngineRender() {
    ...
    $themePath 	= RAMA_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = RAMA_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $ra->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $ra
    ...
    $ra = &$this;
    // First the default Rama themes/functions.php
    include(RAMA_INSTALL_PATH . '/themes/functions.php');
  ...
  /**
   * Draw HTML for a menu defined in $ra->config['menus'].
   *
